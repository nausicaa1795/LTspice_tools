...
%
    ...
    elseif length(strfind( lower(raw_data.plotname), 'noise spectral density' ))~=0,       simulation_type = '.noi';    %SUPPORTED
    end
    
    if length(simulation_type)==0 | ~(strcmpi(simulation_type, '.tran') | strcmpi(simulation_type, '.ac') | strcmpi(simulation_type, '.noi')),
        try fclose( fid );  catch end
    ...
    if strcmpi(file_format, 'binary'),
        ...
        elseif strcmpi( simulation_type, '.noi' ),
            % For Transient Analysis simulations, the time data is stored in double precision floating point binary format, 
            % and everything else is stored in single precision format. 
            ...
            fseek(fid, binary_start, 'bof');    %rewind to start, then extract the time vector.
            raw_data.freq_vect = fread( fid, NumPnts_DS, 'double', (NumVars-1)*4 + (downsamp_N-1)*(NumVars+1)*4, machineformat ).';           
